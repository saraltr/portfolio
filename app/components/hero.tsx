  ...
  return (
    <section className="flex justify-center mx-4 border border-[#7d4f50] main text-[#d8cfc4]">
      <div className="mx-5 w-full max-w-5xl py-10">
        <div
          className="border-4 border-[#7d4f50] bg-[#2b2724] bg-[url('/images/hero.png')] bg-cover bg-center p-6 shadow-[0_0_6px_#7d4f50] rounded-lg"
        ...
        <div className="cursor-pointer mt-10 grid grid-cols-1 md:grid-cols-3 gap-6 text-[#d8cfc4]">
          {[
            { icon: "📌", title: "Latest Projects", desc: "Explore my latest builds", href: "/projects", },
            { icon: "💻", title: "Web Dev Projects", desc: "Interfaces, layouts, and applications ideas", href: "/projects/web-dev" },
            { icon: "🎨", title: "Graphic Design", desc: "Posters, logos, and visual storytelling", href: "/projects/design"},
          ].map((item, i) => (
            <Link href={item.href} key={i}>
            <div
              key={i}
              className="border-2 border-[#7EA680] bg-[#322e2b] p-4 rounded-lg shadow-[0_0_6px_#7EA680] hover:scale-[1.02] transition-transform"
            >
              <div className="text-3xl">{item.icon}</div>
              <h2 className="mt-2 text-xl font-bold text-[#e8bcb9]">{item.title}</h2>
